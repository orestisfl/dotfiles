 ...
 */
/* Based on: https://github.com/vivien/i3blocks-contrib/tree/master/bandwidth2 */
...
static void get_values(char **const ifaces, const int num_ifaces, time_t *const s, unsigned long long int *const received, unsigned long long int *const sent) {
    ...
    unsigned long long int temp_r, temp_s;
    char line[256];
    char ifname[32];
    ...
    *sent = 0;
    while (fgets(line, sizeof(line), f) != NULL) {
        if (sscanf(line, "%31[^:]: %llu %*u %*u %*u %*u %*u %*u %*u %llu", ifname, &temp_r, &temp_s) == 3) {
...
static void display(int const unit, int const divisor,
    ...
    const double one_kb = divisor;
    const double one_mb = divisor * divisor;
    const double ten_mb = 10 * one_mb;
    const double ohd_mb = 100 * one_mb;

    if (b >= ohd_mb) {
        printf("%.0fmb", b / one_mb);
    } else if (b >= ten_mb) {
        printf("%.1fmb", b / one_mb);
    } else if (b >= one_mb) {
        printf("%.2fmb", b / one_mb);
    } else if (b >= one_kb) {
        printf("%.0fkb", b / one_kb);
    } else {
        printf("%.0fb", b);
    }
...
static void parse_ifaces(char *str, char ***ifaces, int *num_ifaces) {
    const int max_ifaces = strlen(str) / 2 + 1;
...
int main(const int argc, char *argv[]) {
    ...
    time_t s_old;
    unsigned long long int received_old, sent_old;
    ...
    while (1) {
        ...
        const double rx = (received - received_old) / (double)(s - s_old);
        const double tx = (sent - sent_old) / (double)(s - s_old);
        printf("%s", label);
