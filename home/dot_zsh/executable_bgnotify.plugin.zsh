...
bgnotify () { ## args: (title, subtitle)
  if hash terminal-notifier 2>/dev/null; then #osx
    ...
    growlnotify -m "$1" "$2"
  elif hash dunstify 2>/dev/null; then #ubuntu gnome!
    (dunstify "$1" "$2" --action 'focus,focus' | grep focus && i3-msg "[id=$((${bgnotify_windowid}))] focus" || true) &>/dev/null &!
...
bgnotify_end() {
  ...
  if (( bgnotify_timestamp > 0 )) && (( past_threshold )); then
    if [ $(currentWindowId) != "$bgnotify_windowid" ]; then
      print -n "\a"
      bgnotify_formatted "$didexit" "$bgnotify_lastcmd" "$elapsed" "$bgnotify_windowid"
    fi
  fi
